                                                ...
                                                std::vector<GEOLIB::Point*>& pnts) const
{
	...
	for (size_t k(0); k < points_inside_polygon.size(); k++)
		perm.push_back(k);
	Quicksort<GEOLIB::PointWithID*> (points_inside_polygon, 0,
	                                 points_inside_polygon.size(), perm);
	...
	for (size_t k(0); k < surface_pnts.size(); k++)
	{
		...
		pnts.push_back ( new GEOLIB::Point ( (*(surface_pnts[k]))[0],
		                                     (*(surface_pnts[k]))[1],
		                                     (*(surface_pnts[k]))[2] ) );
	}
	...
	for (size_t j(0); j < msh_elem_size; j++)
	{
		...
		case MshElemType::TRIANGLE:
		{
			...
			if (k == nodes_indices.Size()) // all nodes of element are points in the surface
				sfc->addTriangle (id_map[nodes_indices[0]],
				                  id_map[nodes_indices[1]],
				                  id_map[nodes_indices[2]]);
			break;
		}
		...
		case MshElemType::TETRAHEDRON:
		{
			...
			{
				size_t n_nodes (msh_elem[j]->GetElementFaceNodes(face_number,
				                                                 face_nodes));
				...
				size_t k;
				for (k = 0; k < n_nodes; k++)
					if (id_map[nodes_indices[face_nodes[k]]] ==
