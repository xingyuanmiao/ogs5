...
{
	std::cout << "GocadInterface::GocadInterface open stream from file " << fname << " ... " <<
	std::flush;
...
//			std::cout << "reading vertex ... " << std::flush;
			...
			_pnt_vec->push_back (new Point (strtod(it++->c_str(),
			                                       &res),
			                                strtod(it++->c_str(),
			                                       &res), strtod(it++->c_str(), &res)));
			id = _pnt_vec->size() - 1;
