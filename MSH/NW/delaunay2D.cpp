...
bool isClosedSurface(Surface* psurface)
{
	...
	for (int i = 1; i < n; i++)
	{
		if (psurface->polyline_of_surface_vector.at(i)->point_vector.front()->id ==
		    endVertexID)
			endVertexID =
			        psurface->polyline_of_surface_vector.at(i)->point_vector.back()->id;
		else if (psurface->polyline_of_surface_vector.at(i)->point_vector.back()->id ==
		         endVertexID)
			endVertexID =
			        psurface->polyline_of_surface_vector.at(i)->point_vector.front()->
			        id;
		else
			return false;
	}
...
bool isOpenBoundary(CGLPolyline* polyline)
{
	...
	std::transform(polyline_name.begin(), polyline_name.end(),
	               polyline_name.begin(), std::toupper);

	const unsigned int postfix_size = (unsigned int)strlen(OPEN_BOUNDARY_POSTFIX);
	if (polyline_name.length() < postfix_size
	    || polyline_name.substr(polyline_name.length() - postfix_size,
	                            postfix_size).compare(OPEN_BOUNDARY_POSTFIX) != 0)
		return false;
	return true;
}
 ...
 **************************************************************************/
int WriteOutputFileBIN(char* filepath,
                       int* node,
                       double* x,
                       double* y,
                       int* nelm,
...
int ExecuteDelaunay2DProcess(char* outfilepath)
{
	...
	if(ReadInputData(&nex, &nbk, &nob, &nib, ibex, (int*)ibno, nidm, ibreak, (int*)nbreak, px,
	                 py, pd))
	{
		triangulate(&kbd,
		            &ktj,
		...
		if (ierrcode == 0)
		{
			...
			else
			{
				...
				WriteOutputFileBIN(bin_out_file_path,
				                   &node,
				                   px,
				                   py,
				                   &nelm,
