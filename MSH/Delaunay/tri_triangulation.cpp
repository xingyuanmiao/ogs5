...
bool bLoadNodeFile_TRI( char* szNodeFile, vector<Cp_dbl3>* pnt )
{
	...
	while( sStatus )
	{
		...
		if (!strncmp(szBuff,"#POINTS\n",7))
			while  (ok == 1)
			{
				fgets(szBuff,STANDARD_BUFFER_SIZE,fp);
				ok = sscanf(szBuff,"%ld", &id);
				if (ok == 1)
				{
					sscanf(szBuff,"%ld %lg %lg %lg",&id,&dX,&dY,&dZ);
...
bool bSaveTriFile_TRI( char* szTriFile, vector<Cp_dtri>* TriList,  vector<Cp_dbl3>* pnt )
{
	bool bRetCode = false;
	FILE* fp = NULL;
	int i, nTri, nPnt;
...
bool bCheckIntersection_TRI(
        ...
        Cp_dbl2* p3 )
{
	if( bCheckPropIntersection_TRI( p0, p1, p2, p3 ) )
		return true;

	if( bBetweenSegments_TRI( p0, p1, p2 ) )
		return true;
	if( bBetweenSegments_TRI( p0, p1, p3 ) )
		return true;
	if( bBetweenSegments_TRI( p2, p3, p0 ) )
		return true;
	if( bBetweenSegments_TRI( p2, p3, p1 ) )
		return true;
...
bool bCheckDiagonalie_TRI( vector<Cp_dbl2>* Pnt2List, int i, int j )
{
	...
	for( k = 0; k < nPoints; k++ )
	{
		...
		if( k1 == j )
			continue;
		if( bCheckIntersection_TRI( Pnt2List, i, j, k, k1 ) )
			return false;
	}
...
bool bCheckDiagonal_TRI( vector<Cp_dbl2>* Pnt2List, int i, int j )
{
	...
	if( !bCheckCone_TRI( Pnt2List, i, j ) )
		return false;
...
{
	bool bRet;
	int i, j, k, nneib = 0, ptr0, ptr1, ptr2, ptr3, ninside;
	int bt, npoints, nright, ptr2_in_trilist;
	short sStatus;
	double dMaxAngle = 0.0, dRadiusStep, dRadius, dDistance, angle, angle2, dx, dy;
	Cp_dtri tri;
	...
	while( true )
	{
		...
		for ( i = 0; i < iBelngCount[ptr1]; i++ )
		{
			...
			if( ptr3 < 0 )
				continue;
			if( bCmpNbrList( p, 1, ptr3,
			                 iNbrList ) && !bCmpNbrList( p, 2, ptr1, iNbrList ) )
			{
			...
			if( iIncluded[ptr3] )
				continue;
			angle = get_3angle( &PntList->at( ptr0 ), &PntList->at(
			                            ptr1 ), &PntList->at( ptr3 ) );
			if( angle < PIE )
			{
				ptr2_in_trilist = true;
				ptr2 = ptr3;
		...
		if( ptr2 < 0 )
		{
			...
			while( sStatus )
			{
				...
				for ( i = 0; i < npoints; i++ )
				{
					...
					if( !iRight[i] )
						continue;
					if( !Circl2.bPointInCircle( &PntList->at( i ) ) )
						continue;
					angle = get_3angle( &PntList->at( ptr1 ), &PntList->at(
					                            i ), &PntList->at( ptr0 ) );
					if( 0 == ninside )
					{
						dMaxAngle = angle;
		...
		if( !ptr2_in_trilist )
		{
			...
			/*----------------------------------------------------------
			#### "|Cgª®·éOp`"XgÌC³B
			#### ptr0Aptr1Aptr2ª®·éOp`ÌªÐÆÂâ³êé
			   ----------------------------------------------------------*/
			iBlgList->Set( ptr0, iBelngCount[ptr0], iTri );
			iBelngCount[ptr0]++;
			iBlgList->Set( ptr1, iBelngCount[ptr1], iTri );
        ...
        vector<Cp_dtri>* TriVector )    // [i/o]
{
	...
	if( 3 == nNbr )
	{
		tri.Set( (int)TriVector->size( ), (int)Chain.at( 0 ), (int)Chain.at(
		                 1 ), (int)Chain.at( 2 ) );
	...
	if( !bPolygonSwapRotation( &Ply2List, true ) )
		goto PIX_EXIT;
