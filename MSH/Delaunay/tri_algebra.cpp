                ...
                int* seq )
{
	...
	if ( left < j )
		DoubleQsx( array, left, j, seq );
	if ( i < right )
		DoubleQsx( array, i, right, seq );
}
        ...
        std::vector<int>* convh )           // [o] convex hull, list of point numbers
{
	...
	for ( i = 0; i < npoints; i++ )
	{
		if ( i == imin )
			...
			angles[i] =
			        get_3angle( &base_point, &Pnt2List->at( imin ), &Pnt2List->at( i ) );
	}
