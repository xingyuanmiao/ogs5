...
int Dtmesh::inputDli(char* fname)
{
	...
	else
	{
		...
		{
			if(flg1)
				return FILE_DLI_TRITETWD;
			else
				return FILE_DLI_TRITET;
		}
		else if(flgtri)
		{
			if(flg1)
				return FILE_DLI_TRIWD;
			else
				return FILE_DLI_TRI;
		}
		else if(flgtet)
