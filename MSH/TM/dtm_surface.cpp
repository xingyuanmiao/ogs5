...
void Surface::setNeighbor()
{
	...
	for(i = 0; i < n; i++)
		for(j = 0; j < 3; j++)
			getTriangle(i)->resetNeighborElement(j,NULL);
				...
				for(k = 0; k < hgsize1; k++)
				{
					...
					for(l = 0; l < 3; l++)
						if(optri->getNode(l) == cr_nd2)
						{
							...
							optri->setNewNeighborElement(ij[l + conect],
							                             crtri);
							//	flg = 1;
							break;
						}
		...
		while(istack.size())
		{
			...
			for(i = 0; i < n; i++)
			{
				...
				if(itri->getMaxlength() <
				   (n0->getDensity() + n1->getDensity()) * rate_density)
					continue;
...
int Surface::swapTriangle(deque<Triangle*>&istack,Node* add_nd,double minarea)
{
	...
	while(istack.size())
	{
		...
		if(new_area1 < minarea || new_area2 < minarea)
			continue;
		if( fabs((new_area1 + new_area2) - (prev_area1 + prev_area2)) > LIMIT )
			continue;
                             ...
                             deque<Triangle*>&nehang)
{
	...
	if(fabs(pre_area - pst_area) > LIMIT)
		return 0;
...
void Surface::swapLawson(deque<Triangle*>istack)
{
	...
	while(istack.size())
	{
		...
		if(lt_tri->getState() > 10)
			continue;
		for(i = 0; i < 3; i++)
		{
			flg = 1;
			if(lt_tri->getNumberOfNeighborElement(i) != 1)
				continue;
			...
			if(new_area1 < minarea || new_area2 < minarea)
				continue;
			if( fabs((new_area1 + new_area2) - (prev_area1 + prev_area2)) > LIMIT_10 )
				continue;
